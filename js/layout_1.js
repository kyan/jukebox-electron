...
function updateCurrentSong(track){
  ...
  time_total.innerHTML = track.duration || '-'
  time_total.setAttribute('data-duration', time_to_seconds(track.duration));
...
function updateCurrentTime(time) {
  if (playState == "play"){
    if (time != undefined) {
      ...
      time = parseInt(time_elapsed.getAttribute('data-time'),10)+1;
    }

    time_elapsed.setAttribute('data-time',time)
    time_elapsed.innerHTML = seconds_to_time(time);

    if (time_total.getAttribute('data-duration') > 0) {
      progress_bar.value = (time / time_total.getAttribute('data-duration')) * 100
    }
  }
}
